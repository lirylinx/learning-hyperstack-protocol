...
async function start() {
    ...
    const core = store.get({name: 'dictionary-english', valueEncoding: 'json'});

    await core.ready();

    const kvPairs = soredDictionaryPairs();
    console.log(`inserido ${kvPairs.length} pairs...`);
    await createDataset(core, kvPairs.slice(0, 1000));
    console.log('Pares inseridos. Querying...');
    ...
    for (const target of targets) {
        ...
        console.log('target %s closest %s', target, closest);
    }
